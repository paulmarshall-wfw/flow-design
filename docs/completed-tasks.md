...
- Task: Add selected-view text editing and document command scaffolding.
  Outcome: Added selected-view text-section editors for View Description and Acceptance Criteria, exposed view text lookups through the core document model, registered title/text-section edits with the document undo manager, and added a focused Flow menu command whose enabled state follows the active document view.
  Verification: sandboxed `swift build` and `swift test` were blocked by Swift module cache permissions; reruns outside the sandbox passed, with `swift test` executing 16 XCTest cases. `python3 scripts/generate_xcode_project.py` regenerated `FlowDesign.xcodeproj`; `xcodebuild ... build` passed; `xcodebuild ... test` passed with 16 XCTest cases; `./scripts/build_and_run.sh --verify` built, launched, and verified the `FlowDesign` process. Xcode emitted the known CoreSimulator out-of-date warnings, but macOS build/test and launch verification passed.
  Traceability: working tree on `a6c9f28`; files changed: `Sources/FlowDesignApp/FlowDesignApp.swift`, `Sources/FlowDesignCore/FlowDesignDocument.swift`, `Tests/FlowDesignCoreTests/FlowDesignDocumentTests.swift`, `README.md`, `docs/completed-tasks.md`, `docs/technical-decisions.md`, and `handoff.md`.
